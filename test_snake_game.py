...
# тест импут скрина
    ...
    def test_grow(self):
        self.game.startGame()
        self.game.snake = Snake([Point(100, 100)], 'UP')
        self.assertEqual(repr(self.game.snake.segments[-1]), "Point(100, 100)")
        initial_length = len(self.game.snake.segments)
    ...
    def test_draw_score(self, MockFont):
        ...
        game.drawScore(mock_surface)

        mock_font.render.assert_called_once_with("Score: 42", True, (255, 255, 255))
        mock_surface.blit.assert_called_once_with(mock_rendered_text, [10, 10])
